  ...
  Widget build(BuildContext context) {
    return Consumer<DynamicUiProvider>(
      builder: (context, dyn, _) {
        ...
        // With no remote background config, the canvas stays flat.
        //
        // This used to render a slowly drifting brand-tinted gradient behind
        // every screen. Under the editorial system that is exactly the wrong
        // move: the canvas has to read as near-black so event photography
        // supplies all the colour, and a moving wash also robbed the scrims
        // on poster cards of their contrast. Remote image/gradient configs
        // below still work - only the default is flat.
                  ...
                  builder: (context, _) {
                    ...
                    final List<Color> colors =
                        (bg.colors.isEmpty || isDarkMode)
                            // Near-canvas neutrals with only a trace of
                            // accent, so a remote gradient still reads as
                            // paper rather than a brand wash.
                            ? [
                              scheme.canvas,
                              scheme.surface,
                              scheme.accent.withValues(alpha: 0.05),
                            ]
                            : bg.colors
                                .map(_parseColor)
                                .whereType<Color>()
              ...
              if (bg.hasImage)
                Stack(
                  ...
                  children: [
                    ...
                    // Light-mode wash over a remote background image. Kept
                    // to a single faint accent - a three-hue wash was what
                    // made light mode read as a different product.
                    if (!isDarkMode)
                      Container(
                        decoration: BoxDecoration(
                          gradient: LinearGradient(
                            ...
                            colors: [
                              ...
                              AppPalette.paper.withValues(alpha: 0.10),
                            ],
                          ),
                        ),
                      ),
