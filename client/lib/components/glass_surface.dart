...
/// An editorial panel: near-solid surface, hairline border, deep shadow.
///
/// In the dark editorial system the canvas is almost black, so a heavily
/// translucent "glass" panel would disappear into it. These surfaces stay
/// mostly opaque and rely on a 1px hairline for their edge; the blur is
/// retained only for chrome that floats over scrolling content.
class GlassSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Near-solid so the panel reads against a near-black canvas.
    final Color baseTint =
        tintColor ?? (isDark ? AppPalette.surface : AppPalette.paperSurface);
    final double baseAlpha = isDark ? 0.94 : 0.98;
    final Color resolvedTint =
        tintColor != null ? baseTint : baseTint.withValues(alpha: baseAlpha);

    // The hairline carries the edge, at full strength.
    final Color resolvedBorder = borderColor ?? scheme.hairline;
...
class GradientGlassSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bgColor =
        backgroundColor ??
        (isDark ? AppPalette.surface : AppPalette.paperSurface);
...
class FrostedSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final resolvedColor =
        color ?? (isDark ? AppPalette.surfaceMuted : AppPalette.paperMuted);
